...
/**
 * La classe Equipage represente un equipage, elle permet d'effectuer
 * differentes actions sur les pirates dont les principales definir le nombre de
 * pirates de l'equipage et les relations entre eux ainsi que d'attribuer a
 * chaque pirate l'objet recu apres la repartition des biens selon
 * l'utilisateur.
 * 
 * @author Salas RADJEF
...
public class Equipage {
				...
				for (String key : deteste.keySet()) {
					...
					if (tst != null) {
						for (int i = 0; i < tst.size(); i++) {
							...
							// on verifie que les 2 pirates de la relation deteste existent avant de les
							// ajouter.
							if (a == null | b == null) {
	 ...
	 * 
	 * @param A : est de type Pirate. C'est le pirate pour lequel nous cherchons la
	 *          position dans l'equipage
	...
	public void addRelation(Pirate A, Pirate B) {
		...
		if (validation) {
			...
			// System.out.println("La relation ne s'aiment pas entre " + A.getID() + " et "
			// + B.getID() +" a bien ete ajoutee" );
		} else {
	...
	/**
	 * La methode findPirateByID permet de recuperer un pirate a partir de son ID
	 * (l'ID d'un pirate passe en parametre) un pirate se trouve dans dans
	 ...
	 * @return l'objet pirate dans le cas ou nous l'avons trouve qu'il appartient a
	...
	/**
	 * La methode affectationNaive permet d'affecter de facon naive et chaque pirate
	 * un objet parmi ceux qu'il y a a partager en fonction des preferences de
	 * chacun. Chaque pirate sera associe a un objet et sera represente dans une
	 * HashMap <Pirate,objet>
	 */
	...
	/**
	 * La methode affectationStupide permet d'affecter les objets au pirates dans
	 * l'ordre inverse que celui dans lequel nous les avons enregistrer dans
	 * l'attribut equipage a savoir c'est le dernier pirate de l'attribut equipage
	 * qui aura en premier son objet prefere et les autres auront leurs objets
	 * prefere selon ceux qui ne sont pas encore pris
	 */
	...
	/**
	 * La methode changerObjet permet a l'utilisateur d'echanger les objets entre
	 * deux pirates en utilisant la HashMap
	 *
	 * @param A represente le pirate qui a l'un des objets a echanger
	 * @param B represente le pirate avec qui il va echanger objet avec le pirate du
	 *          premier parametre
	 */
	...
	/**
	 * La methode calcule le nombre de pirates jaloux dans l'ï¿½quipage aprï¿½s
	...
	public HashMap<Pirate, String> getObjet_recu() {
		return this.objet_recu;
	}
}