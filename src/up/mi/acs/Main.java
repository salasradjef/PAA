...
public class Main {
				...
				try {
					...
					if (reponse == 3) {
						try {
							...
							System.err.println(
									"Une erreur s'est produite lors de l'ecriture dans le fichier de sauvegarde.");
						}
					}
					...
					System.err.println("parametres incorrectes");
				}
			}

			sc.close();
		} else {
			System.err.println("Le fichier passe en parametres est mal syntaxe");
		}
	}
	...
	public static int Menu(Scanner sc) {
		...
		try {
			...
			System.out.println("La reponse que vous avez entre ne correspond pas a un choix possible");
		}
		switch (key) {
		case 1:
			return 1;
	 ...
	 * 
	 * 
	 * ArrayList<Pirate> added = new ArrayList<>(); ArrayList<Pirate> equipagee =
	 * equipage.getEquipage(); HashMap<Pirate,Integer> costs = new HashMap<>();
	 * 
	 * 
	 * while(added.size() != equipagee.size()){
	 * 
	 * Pirate PirateRandom = equipage.getEquipage().get(new
	 * Random().nextInt(equipage.getEquipage().size())); if (!isIn(PirateRandom,
	 * added)) { ArrayList<Pirate> visited = new ArrayList<>();
	 * visited.add(PirateRandom); equipage.affectationNaive(); ArrayList<Pirate>
	 * deteste = new ArrayList<>(); for (int j = 0; j < equipagee.size(); j++) { if
	 * (equipage.hateRelation(PirateRandom, equipagee.get(j))) {
	 * deteste.add(equipagee.get(j)); } } ArrayList<Integer> litle_costs = new
	 * ArrayList<>(); //tanque j'ai pas visité tout les sommets voisins de notre
	 * sommet X je continue while (visited.size() != deteste.size()) { Pirate
	 * VoisinRandom = deteste.get(new Random().nextInt(deteste.size()));
	 * if(VoisinRandom != null){ if (!isIn(VoisinRandom, visited)) {
	 * equipage.changerObjet(PirateRandom, VoisinRandom); }
	 * 
	 * litle_costs.add(equipage.cost()); visited.add(VoisinRandom); }
	 * 
	...
	/*
	 * public static void resNul(Equipage equipage){ ArrayList<String> deja_test =
	 * new ArrayList<>(); equipage.affectationStupide(); ArrayList<Pirate> equipagee
	 * = equipage.getEquipage(); int S = equipage.cost(); for(int
	 * j=0;j<equipagee.size();j++){ Pirate ts = equipagee.get(j);
	 * 
	 * for(int i=0;i<equipage.getObjets().size();i++){ String obj =
	 * equipage.getObjets().get(i); Pirate tmp = null; if(!isIn(obj,deja_test)){ tmp
	 * = equipage.whichPirateUseMyObject(obj); equipage.changerObjet(ts,tmp); } int
	 * S2 = equipage.cost();
	 * 
	 * if(S2 < S){ S = S2; }else{ equipage.changerObjet(tmp,ts); }
	 * 
	...
	public static Equipage ResManuelle(Equipage equipage, Scanner sc) throws IllegalArgumentException {
		...
		while (!termine2) {
			...
			for (int i = 0; i < equipage.getNombrePirate(); i++) {
				System.out.println(equipage.getEquipage().get(i).getID() + ":"
						+ equipage.getObjet_recu().get(equipage.getEquipage().get(i)));
			}
			// affiche les objets que possede chaque pirate apres chaque action.
			// Nous avons determines lors de la lecture du sujet qu'il n'etait pas
			// necessaire de le faire apres l'affectation naive,
			// car il serait logique de s'interesser au cout de la solution initiale avant
			// d'effectuer les echanges d'objets.

			if (reponse2 == 3) {
				termine2 = true;
				System.out.println("Vous quittez le menu de resolution manuelle.");
	...
	public static void Sauvegarder(Scanner sc, Equipage equipage) throws IOException {
		System.out
				.println("Quel est le nom du fichier dans lequel vous souhaitez enregistrer la sauvegarde actuelle ?");
		...
		while (true) {
			try {
				...
				System.err.println("Le nom du fichier que vous avez saisie existe deja ");
				nomFile = sc.next();
			}
		}
